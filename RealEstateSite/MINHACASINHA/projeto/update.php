...
if(isset($_POST['submit'])){
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         $warning_msg[] = 'Confirme a palavra-passe não combinada';
      }else{
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `users` SET password = ? WHERE id = ?");
            $update_pass->execute([$c_pass, $user_id]);
            $success_msg[] = 'Palavra-passe atualizada com sucesso!';
         }else{
            $warning_msg[] = 'Por-favor insira uma nova palavra-passe!';
         }
      }
   }
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <input type="password" name="old_pass" maxlength="20" placeholder="Coloque a sua palavra-passe antiga" class="box">
      <input type="password" name="new_pass" maxlength="20" placeholder="Insira a sua nova palavra-passe" class="box">
      <input type="password" name="c_pass" maxlength="20" placeholder="Confirme a sua nova palavra-passe" class="box">
