...
if(isset($_POST['delete'])){
   ...
   if($verify_delete->rowCount() > 0){
      ...
      $success_msg[] = 'Solicitação removida com sucesso!';
   }else{
      $warning_msg[] = 'Solicitação já foi removida!';
   }
...
<section class="requests">
   ...
   <h1 class="heading">Todas as solicitações</h1>
   ...
   ?>
   <div class="box">
      <p>Nome : <span><?= $fetch_sender['name']; ?></span></p>
      ...
      <p>Inquérito para : <span><?= $fetch_property['property_name']; ?></span></p>
      <form action="" method="POST">
         <input type="hidden" name="request_id" value="<?= $fetch_request['id']; ?>">
         <input type="submit" value="Remover solicitação" class="btn" onclick="return confirm('Remover esta solicitação?');" name="delete">
   ...
   <?php
      ...
      echo '<p class="empty">Você não tem solicitações!</p>';
   }
   ?>
